"""
classifier.py — Classifie les joueurs Clash Royale par niveau d activite.
...
Status:
  hot (>=100) / active (>=60) / warm (>=25) / cold (>0) / unknown (0)
"""
...
def classify_player(
    ...
    """
    Calcule activity_score, activity_status et next_scan_at.

    Parametres:
        new_profile  : donnees fraiche de l API (ou profil partiel membre clan)
        old_profile  : ancien enregistrement DB pour comparer les deltas
        battlelog    : liste des batailles recentes
        clan_member  : donnees membres clan si pas de profil complet

    Retourne:
        {"activity_score": int, "activity_status": str,
         "next_scan_at": str ISO, "last_battle_time": str|None}
    """
    score = 0
    now = _now()

    # --- lastSeen depuis profil ou membre clan
    ...
    # --- dernier battleTime depuis battlelog
    ...
    # --- deltas vs ancien profil
    if old_profile:
        new_bc = new_profile.get("battleCount", 0) or 0
        old_bc = old_profile.get("battle_count", 0) or 0
        if new_bc > old_bc:
            score += 80

        new_tr = new_profile.get("trophies", 0) or 0
        old_tr = old_profile.get("trophies", 0) or 0
        if new_tr != old_tr:
            score += 25

        new_don = new_profile.get("donations", 0) or 0
        old_don = old_profile.get("donations", 0) or 0
        if new_don != old_don:
            score += 20
...
def compute_next_scan_at(status: str) -> str:
    """Retourne la prochaine date de scan ISO UTC selon le status."""
    now = _now()
    deltas = {
